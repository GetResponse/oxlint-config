...
const ruleNameCell = (rule) => {
    ...
    const scope = rule.scope && rule.scope !== '*'
        ? `<div class="scope">${escape(rule.scope)}</div>`
        : '';

    return `${name} ${source}${scope}`;
};
...
const STYLES = `
    ...
    .scope { color: #888; margin-top: 4px; font-size: 12px; font-family: "SF Mono", Menlo, Consolas, monospace; }
...
export function renderHtml({ enabled }) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>@getresponse/oxlint-config — rules</title>
<style>${STYLES}</style>
</head>
<body>
